...
fn main() {
    ...
    let (encoded_verifying_key, message, signature, poap_index): (EncodedPoint, Vec<u8>, Signature, U256) =
        env::read();
    ...
    let call: POAP::tokenDetailsOfOwnerByIndexCall =
        POAP::tokenDetailsOfOwnerByIndexCall { owner: CALLER, index:poap_index };
    ...
    let returns = ViewCall::new(call, CONTRACT).with_caller(CALLER).execute(view_call_env).clone();
